...
modularity_limit = np.max(G.base_mod_N)
...
cut = np.sign(ul2).T @ (np.diag(G.deg) - G.A) @ np.sign(ul2)
modularity_limit = vl2

lambdas = np.hstack(
    (
        lambdas,
        np.array([[va1], [vll2], [vl2], [cut]]),
    )
)
...
if graphtype == "ER":
    st = fig.suptitle(
        "ER; n=" + str(n) + "; p=" + str(round(p, 2)) + "; seed =" + str(seed),
        fontsize="x-large",
    )
elif graphtype == "BA":
    st = fig.suptitle(
        "BA; n=" + str(n) + "; m=" + str(kn) + "; seed =" + str(seed),
        fontsize="x-large",
    )
while True:
    switches += 1

    swt = G.modAwareSwitch(modularity_limit, small_switch_limit=0, normalized=True)
    if swt == (-1, -1, -1, -1):
        break
    ...
    im3 = ig.plot(
        ig.Graph.Adjacency(G.A),
        vertex_size=np.log(G.deg) * (50 / np.log(G.deg)[0]),
    ...
    (im11,) = ax1.plot(
        [-1e9, 1e9], [0, 0], label="", color="k", linestyle=":", linewidth=0.1
    )
    (im12,) = ax1.plot(
        (lambdas[0, :] - lambdas[0, 0]) / lambdas[0, 0], label="L1A", color="tab:orange"
    )
    (im14,) = ax1.plot(
        (lambdas[1, :] - lambdas[1, 0]) / lambdas[1, 0],
        label="L2L",
        color="tab:orange",
        linestyle="-.",
    )
    (im13,) = ax1.plot(
        (lambdas[2, :] - lambdas[2, 0]) / lambdas[2, 0],
        label="L2B",
        color="tab:orange",
        linestyle=":",
    )
    (im15,) = ax1.plot(
        (lambdas[3, :] - lambdas[3, 0]) / lambdas[3, 0],
        label="Cut",
        color="tab:orange",
        linestyle="--",
    )
