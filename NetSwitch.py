...
class NetSwitch(NetMat):
    ...
    def __init__(self, ig_graph, checker_count=False, pos_only=True):
        ...
        self.pc, self.nc = -1, -1
        self.pc_rows, self.nc_rows = -1, -1

        if self.checker_count:
            self.checkercount_matrix()  # Counts the checkerboards for all pairs of nodes
        self.swt_done = 0

    def sort_adj(self):
        sort_idxs = np.argsort(-self.deg)
        self.A = self.A[sort_idxs, :][:, sort_idxs]
        self.deg = self.deg[sort_idxs]
    ...
    def sample_uniform(self, alg='RAND'):
        match alg:
            case 'RAND':
                raise 'Not yet!'
            case 'CurveBall':
                raise 'Not yet!'
            case _:
                raise 'Undefined Sampling Algorithm!'
