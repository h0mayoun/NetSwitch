...
class NetSwitch:
            ...
            match alg:
                ...
                case "SWOP":
                    if self.swt_done == 0:
                        ...
                        #self.m_limit = self.Modul(normed=False)
                    cswitch_found = False
                    while not cswitch_found:
                        if self.total_checkers() == 0:
                            break
                        ...
                        for curk, curl in all_kls:
                            ...
                            if not min(
                                ...
                                self.switch(swt, update_N=False)
                                print(self.m_limit, max(self.base_mod))
                                self.update_N(swt)
                                cswitch_found = True
                                break
                            else:
    ...
    def l2(self, normed=True):
        try:
            ...
            if normed:
                eigVal, eigVec = np.linalg.eig(self.normalized_laplacian())
                idx = eigVal.argsort()
                return eigVal[idx[1]]
            else:
                eigVal, eigVec = np.linalg.eig(self.laplacian())
                idx = eigVal.argsort()
                return eigVal[idx[1]]
    ...
    def Mlev(self, normed=True):
        if normed:
            Dm05 = np.diag(1 / np.sqrt(self.deg))
            M_n = np.matmul(np.matmul(Dm05, self.M), Dm05)
            eig_val = eigsh(
                M_n.astype(float), k=1, which="LA", return_eigenvectors=False
            )[0]
        else:
            eig_val = eigsh(
    ...
    def Modul(self, normed=True):
        if normed:
            Dm05 = np.diag(1 / np.sqrt(self.deg))
            M_n = np.matmul(np.matmul(Dm05, self.M), Dm05)
            eig_val, eig_vec = eigsh(M_n.astype(float), k=1, which="LA")[0]
            eig_vec = np.sign(eig_vec) / np.sqrt(self.n)
            return (eig_vec.T @ M_n @ eig_vec)[0, 0]
        else:
            eig_val, eig_vec = eigsh(self.M.astype(float), k=1, which="LA")
            eig_vec = np.sign(eig_vec) / np.sqrt(self.n)
