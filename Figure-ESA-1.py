...
figa, axa = plt.subplots(2, 1, figsize=(6, 6))
...
axa[0].plot(swt, m2, label=r"GRDY", c="#e31a1c")
axb.plot(swt, lev, label=r"$x=\mathbf{x}_1$", c="#fdbf6f")
axb.plot(swt, levhat, label=r"$x=d\|d\|^{-1}$", c="#a6cee3")
...
axa[1].set_xlim([0, S.swt_done])

swt = np.array([i[0] for i in data])
lev = np.array([i[1] for i in data])
m2 = np.array([i[2] for i in data])
print(np.mean([i[4] for i in data]))
axa[0].plot(swt, m2, label=r"RAND", c="#33a02c")
...
axa[1].set_xlabel("Switches done")
axb.set_xlabel("Switches done")

axa[0].set_ylabel("Degree Assortativity")
axa[1].set_ylabel("Time(s)")
axb.set_ylabel(r"$x^TAx$")


figa.tight_layout()
figa.subplots_adjust(top=0.88)
figb.tight_layout()
figb.subplots_adjust(top=0.88)

axa[0].spines["top"].set_visible(False)
axa[0].spines["right"].set_visible(False)
axa[1].spines["top"].set_visible(False)
axa[1].spines["right"].set_visible(False)
axb.spines["top"].set_visible(False)
axb.spines["right"].set_visible(False)

figa.suptitle("Random versus greedy positive switch")
axa[1].set_yscale("log")
axa[0].legend(frameon=False, loc="lower right")
axb.set_title("Evolution of spectral radius")
axb.legend(frameon=False, loc="lower right")

figa.savefig("ESA-Fig-1-a.pdf".format(graph_des), dpi=300)
figb.savefig("ESA-Fig-1-b.pdf".format(graph_des), dpi=300)
